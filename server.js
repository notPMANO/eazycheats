...
// Optional owner backup key. Set MASTER_KEY in the environment (kept a secret,
// since this repo is public) to have one always-valid key for yourself — no DB
// record, expiry, HWID, or per-game gate. Unset = disabled.
function validateKeyAuth(key, hwid, requiredGameId) {
  if (process.env.MASTER_KEY && key === process.env.MASTER_KEY) return { ok: true };
